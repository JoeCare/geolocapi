...
from .factorial_tests import CompanyFactory, UserFactory
...
class CompanyViewSetTestCase(TestCase):
	...
	def test_unauthenticated(self):
		...
		with self.subTest('PUT'):
			...
			response = self.client.put(self.get_detail_url(company.id),
									   data=data)
			self.assertEqual(response.status_code,
							 status.HTTP_403_FORBIDDEN)
		...
		with self.subTest('PATCH'):
			data = {'name': 'New name'}
			response = self.client.patch(self.get_detail_url(company.id),
										 data=data)
